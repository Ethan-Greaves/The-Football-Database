...
router.get(`/`, async (req, res, next) => {
	...
	try {
		if (req.user) favourites = req.user.favourites;
		...
		for (let i = 0; i < favouritesToDisplay.length; i++) {
			if (favouritesToDisplay[i].players) {
				if (
					favouritesToDisplay[i].players[0].strTeam !== '_Retired Soccer' &&
					favouritesToDisplay[i].players[0].strTeam !== '_Free Agent Soccer' &&
					favouritesToDisplay[i].players[0].strTeam !== '_Deceased Soccer'
				) {
					const team = await requestDataFromAPI(
						`https://www.thesportsdb.com/api/v1/json/1/searchteams.php?t=`,
						favouritesToDisplay[i].players[0].strTeam
					);
			...
			if (favouritesToDisplay[i].teams) {
				const team = await requestDataFromAPI(
					`https://www.thesportsdb.com/api/v1/json/1/searchteams.php?t=`,
					favouritesToDisplay[i].teams[0].strTeam
				);
